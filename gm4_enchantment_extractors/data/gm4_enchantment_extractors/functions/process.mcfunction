...
execute if data storage gm4_custom_crafters:temp/crafter {Items:[]} positioned ~ ~1 ~ as @e[type=item,distance=..0.7,nbt={Item:{Count:1b,tag:{Enchantments:[{}]}}}] if data entity @s Item.tag.Enchantments[0].id positioned ~ ~-2 ~ run function gm4_enchantment_extractors:extract
